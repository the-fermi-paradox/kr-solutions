...
int get_distance(int i) {
  ...
  if (colwidth) {
    return (colwidth - ((i - strtcol) % colwidth)); 
  }
...
void prune(char* from, char* to) {
  ...
  while (*from) {
    ...
    do {
      *to++ = '*';
    } while ((to - torig) < strtcol ||
             (colwidth && (to - torig) >= strtcol && (to - torig) % colwidth) ||
             (stor < stp && (to - torig) < *stor) ||
             (stor >= stp && (to - torig) % TAB_STOP));
